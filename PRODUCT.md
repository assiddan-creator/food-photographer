...
- **Restaurant settings («הגדרות מסעדה»):** header entry from the kitchen. Mock chrome: חזרה, title, subtitle «וואטסאפ · גוגל · נאמנות — בלי קלוריות», cards for WhatsApp (number + business name), Google review link + «בדיקת קישור», locked loyalty cards **לקוח ראשון · 10%** and **לקוח חוזר · 12%** with subtitle «קבוע — לא בחירה חופשית», optional first/returning copy fields. Saved in `localStorage` (env `NEXT_PUBLIC_WHATSAPP` / `NEXT_PUBLIC_GOOGLE_REVIEW_URL` are fallbacks). WhatsApp «מבצע היום» / «לפעם הבאה» pull 10% first / 12% returning. Google review template is request + link only — **no discount** (Google policy). No discount-for-stars or discount-for-likes.
